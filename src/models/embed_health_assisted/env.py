...
class Env(VsHeuristicEnv):
    def __init__(self, *args, **kwargs):
        ...
        self.buffer = defaultdict(lambda: 6)

    def calc_reward(self, _, current_battle: Gen8Battle) -> float:
        opp_team = current_battle.opponent_team.values()
        total_hp_frac = 6 - len(opp_team) + sum(p.current_hp_fraction for p in opp_team)

        delta = self.buffer[current_battle] - total_hp_frac
        self.buffer[current_battle] = total_hp_frac
